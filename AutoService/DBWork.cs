...
namespace AutoService
{
    internal class DBWork
    {
        ...
        public void AddMechanic(string name)
        {
            ...
            {
                mechanics.Add(name);
                Console.WriteLine($"Механик {name} добавлен.");
            }
            else
            {
                Console.WriteLine($"Механик с именем {name} уже существует.");
            }
        }

        public void RemoveMechanic(string name)
        {
            if (mechanics.Remove(name))
            {
                Console.WriteLine($"Механик {name} удалён.");
            }
            else
            {
                Console.WriteLine($"Механик с именем {name} не найден.");
            }
        }

        public void EditMechanic(string oldName, string newName)
        {
            int index = mechanics.IndexOf(oldName);
            if (index != -1)
            {
                mechanics[index] = newName;
                Console.WriteLine($"Механик {oldName} изменён на {newName}.");
            }
            else
            {
                Console.WriteLine($"Механик с именем {oldName} не найден.");
            }
        }
